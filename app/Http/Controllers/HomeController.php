...
class HomeController extends Controller
{
    ...
    public function publicTimeline($nickname) {
        ...
        if ($user) {
            ...
            return view('shoutpublic',['status' => $status,'avatar' => $avatar,'name' => $name, 'displayActions' => $displayActions, 'friendId' => $user->id]);
        } else {
            return redirect('/');
        }
    }
    ...
    function saveProfile(Request $request){
        if(Auth::check()) {
            ...
            $profileImage = "user".$user->id .".".$request->profileimage->extension();
            $request->profileimage->move(public_path('images'), $profileImage);
